...
(defrule find_seating "find seating for guest"
   ...
   ?seating <- (seating (seat2 ?s2) (name2 ?n2) (id ?id) (path_done TRUE) )
...
(defrule print_results "print the results"
  ...
  (last_seat (seat ?s2) )
  ?path <- (path (id ?id) (name ?n1) (seat ?s1) )
=>
...
(assert (count (c 1) ) )
(load-facts ./benchmark/manners/128guest.dat)
(bind ?*start* (ms-time) )
(bind ?fcount (fire) )
(bind ?*end* (ms-time) )
(bind ?et (- ?*end* ?*start*) )
(printout t "elpasedTime=" ?et crlf)
(printout t "rules fired: " ?fcount crlf)